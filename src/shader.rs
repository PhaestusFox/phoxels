...
use bevy::color::{Alpha, Color, ColorToComponents};
use bevy::math::{Affine2, Affine3, Mat2, Mat3, Vec2, Vec3, Vec4};
use bevy::pbr::{OpaqueRendererMethod, StandardMaterialKey};
use bevy::prelude::*;
...
///
/// It is used for every texture in the [`CustomMaterial`].
...
#[data(0, CustomMaterialUniform, binding_array(10))]
...
pub struct CustomMaterial {
    ...
    /// The UV channel to use for the [`CustomMaterial::base_color_texture`].
    ///
    ...
    ///
    /// [`base_color`]: CustomMaterial::base_color
    #[texture(1)]
    #[sampler(2)]
    #[dependency]
    ...
    /// The UV channel to use for the [`CustomMaterial::emissive_texture`].
    ///
    /// Defaults to [`UvChannel::Uv0`].
    ...
    ///
    /// [`emissive`]: CustomMaterial::emissive
    #[texture(3)]
    #[sampler(4)]
    #[dependency]
    ...
    /// The UV channel to use for the [`CustomMaterial::metallic_roughness_texture`].
    ///
    /// Defaults to [`UvChannel::Uv0`].
    ...
    ///
    /// [`metallic`]: CustomMaterial::metallic
    /// [`perceptual_roughness`]: CustomMaterial::perceptual_roughness
    #[texture(5)]
    #[sampler(6)]
    #[dependency]
    ...
    /// A color with which to modulate the [`CustomMaterial::reflectance`] for
    /// non-metals.
    ///
    ...
    ///
    /// For specular transmission usecases with refraction (e.g. glass) use the [`CustomMaterial::specular_transmission`] and
    /// [`CustomMaterial::ior`] properties instead.
    ///
    ...
    /// ## Notes
    ///
    /// - The material's [`CustomMaterial::base_color`] also modulates the transmitted light;
    ...
    /// The UV channel to use for the [`CustomMaterial::diffuse_transmission_texture`].
    ///
    /// Defaults to [`UvChannel::Uv0`].
    #[cfg(feature = "pbr_transmission_textures")]
    pub diffuse_transmission_channel: UvChannel,

    /// A map that modulates diffuse transmission via its alpha channel. Multiplied by [`CustomMaterial::diffuse_transmission`]
    /// to obtain the final result.
    ///
    /// **Important:** The [`CustomMaterial::diffuse_transmission`] property must be set to a value higher than 0.0,
    ...
    /// The material's [`CustomMaterial::base_color`] also modulates the transmitted light.
    ///
    /// **Note:** Typically used in conjunction with [`CustomMaterial::thickness`], [`CustomMaterial::ior`] and [`CustomMaterial::perceptual_roughness`].
    ///
    /// ## Performance
    ///
    ...
    /// - If purely diffuse light transmission is needed, (i.e. “translucency”) consider using [`CustomMaterial::diffuse_transmission`] instead,
    ...
    /// The UV channel to use for the [`CustomMaterial::specular_transmission_texture`].
    ///
    /// Defaults to [`UvChannel::Uv0`].
    #[cfg(feature = "pbr_transmission_textures")]
    pub specular_transmission_channel: UvChannel,

    /// A map that modulates specular transmission via its red channel. Multiplied by [`CustomMaterial::specular_transmission`]
    /// to obtain the final result.
    ///
    /// **Important:** The [`CustomMaterial::specular_transmission`] property must be set to a value higher than 0.0,
    /// or this texture won't have any effect.
    ...
    ///
    /// **Note:** Typically used in conjunction with [`CustomMaterial::specular_transmission`] and [`CustomMaterial::ior`], or with
    /// [`CustomMaterial::diffuse_transmission`].
    #[doc(alias = "volume")]
    #[doc(alias = "thin_walled")]
    pub thickness: f32,

    /// The UV channel to use for the [`CustomMaterial::thickness_texture`].
    ///
    /// Defaults to [`UvChannel::Uv0`].
    #[cfg(feature = "pbr_transmission_textures")]
    pub thickness_channel: UvChannel,

    /// A map that modulates thickness via its green channel. Multiplied by [`CustomMaterial::thickness`]
    /// to obtain the final result.
    ///
    /// **Important:** The [`CustomMaterial::thickness`] property must be set to a value higher than 0.0,
    /// or this texture won't have any effect.
    ...
    /// | Moissanite      | 2.65                 |
    ///
    /// **Note:** Typically used in conjunction with [`CustomMaterial::specular_transmission`] and [`CustomMaterial::thickness`].
    ...
    /// - [`CustomMaterial::attenuation_color`];
    /// - [`CustomMaterial::thickness`];
    /// - [`CustomMaterial::diffuse_transmission`] or [`CustomMaterial::specular_transmission`].
    ...
    ///
    /// **Note:** To have any effect, must be used in conjunction with:
    /// - [`CustomMaterial::attenuation_distance`];
    /// - [`CustomMaterial::thickness`];
    /// - [`CustomMaterial::diffuse_transmission`] or [`CustomMaterial::specular_transmission`].
    ...
    /// The UV channel to use for the [`CustomMaterial::normal_map_texture`].
    ///
    /// Defaults to [`UvChannel::Uv0`].
    ...
    /// # Notes
    ///
    /// Normal mapping with `CustomMaterial` and the core bevy PBR shaders requires:
    ...
    /// The UV channel to use for the [`CustomMaterial::occlusion_texture`].
    ///
    /// Defaults to [`UvChannel::Uv0`].
    ...
    /// The UV channel to use for the [`CustomMaterial::specular_texture`].
    ///
    /// Defaults to [`UvChannel::Uv0`].
    ...
    /// [`CustomMaterial::reflectance`] value. This follows the
    ...
    /// [`CustomMaterial::reflectance`] value to 2.0 so that this map can
    /// express the full [0.0, 1.0] range of values.
    ///
    /// Note that, because the reflectance is stored in the alpha channel, and
    /// the [`CustomMaterial::specular_tint_texture`] has no alpha value, it
    ...
    /// [`CustomMaterial::specular_tint_texture`].
    ///
    /// Defaults to [`UvChannel::Uv0`].
    #[cfg(feature = "pbr_specular_textures")]
    ...
    /// [`CustomMaterial::specular_tint`] value. See the documentation for
    ...
    /// The UV channel to use for the [`CustomMaterial::clearcoat_texture`].
    ///
    /// Defaults to [`UvChannel::Uv0`].
    ...
    /// main [`CustomMaterial::clearcoat`] factor.
    ///
    ...
    /// the same way as the [`CustomMaterial::perceptual_roughness`].
    ///
    /// If the [`CustomMaterial::clearcoat`] value if zero, this has no
    /// effect.
    ///
    /// Defaults to 0.5.
    pub clearcoat_perceptual_roughness: f32,

    /// The UV channel to use for the [`CustomMaterial::clearcoat_roughness_texture`].
    ///
    /// Defaults to [`UvChannel::Uv0`].
    #[cfg(feature = "pbr_multi_layer_material_textures")]
    ...
    /// [`CustomMaterial::clearcoat_perceptual_roughness`] factor.
    ///
    /// As this is a non-color map, it must not be loaded as sRGB.
    ...
    /// The UV channel to use for the [`CustomMaterial::clearcoat_normal_texture`].
    ///
    /// Defaults to [`UvChannel::Uv0`].
    #[cfg(feature = "pbr_multi_layer_material_textures")]
    ...
    /// format as [`CustomMaterial::normal_map_texture`].
    ///
    ...
    /// in both [`CustomMaterial::normal_map_texture`] and this field.
    ///
    /// As this is a non-color map, it must not be loaded as sRGB.
    ...
    /// [`CustomMaterial::anisotropy_rotation`] of 0) aligns with the
    ...
    /// the [`CustomMaterial::anisotropy_rotation`] parameter.
    ///
    ...
    /// [`CustomMaterial::anisotropy_strength`]. The rotation is applied in
    ...
    /// [`CustomMaterial::anisotropy_strength`] is zero. Its value can
    /// optionally be adjusted across the mesh with the
    /// [`CustomMaterial::anisotropy_texture`].
    ///
    /// See the [`KHR_materials_anisotropy` specification] for more details.
    ///
    /// [`KHR_materials_anisotropy` specification]:
    ...
    /// The UV channel to use for the [`CustomMaterial::anisotropy_texture`].
    ///
    /// Defaults to [`UvChannel::Uv0`].
    ...
    /// [`CustomMaterial::anisotropy_strength`] and
    /// [`CustomMaterial::anisotropy_rotation`] to vary across the mesh.
    ///
    ...
    /// [`CustomMaterial::anisotropy_rotation`] parameter is applied. The
    /// value in the blue channel is multiplied by the
    /// [`CustomMaterial::anisotropy_strength`] value to produce the final
    /// anisotropy strength.
    ///
    ...
    /// [`parallax_depth_scale`]: CustomMaterial::parallax_depth_scale
    /// [`parallax_mapping_method`]: CustomMaterial::parallax_mapping_method
    /// [`max_parallax_layer_count`]: CustomMaterial::max_parallax_layer_count
    #[texture(11)]
    #[sampler(12)]
    #[dependency]
...
impl Default for CustomMaterial {
    fn default() -> Self {
        CustomMaterial {
            ...
            deferred_lighting_pass_id: bevy::pbr::deferred::DEFAULT_PBR_DEFERRED_LIGHTING_PASS_ID,
...
bitflags::bitflags! {
    ...
    /// This is accessible in the shader in the [`CustomMaterialUniform`]
    #[repr(transparent)]
...
pub struct CustomMaterialUniform {
    ...
    /// The [`CustomMaterialFlags`] accessible in the `wgsl` shader.
    pub flags: u32,
    /// When the alpha mode mask flag is set, any base color alpha above this cutoff means fully opaque,
    /// and any below means fully transparent.
    pub alpha_cutoff: f32,
    /// The depth of the [`CustomMaterial::depth_map`] to apply.
    pub parallax_depth_scale: f32,
...
impl AsBindGroupShaderType<CustomMaterialUniform> for CustomMaterial {
    fn as_bind_group_shader_type(&self, images: &RenderAssets<GpuImage>) -> CustomMaterialUniform {
        let mut flags = CustomMaterialFlags::NONE;
        if self.base_color_texture.is_some() {
            flags |= CustomMaterialFlags::BASE_COLOR_TEXTURE;
        }
        if self.emissive_texture.is_some() {
            flags |= CustomMaterialFlags::EMISSIVE_TEXTURE;
        }
        if self.metallic_roughness_texture.is_some() {
            flags |= CustomMaterialFlags::METALLIC_ROUGHNESS_TEXTURE;
        }
        if self.occlusion_texture.is_some() {
            flags |= CustomMaterialFlags::OCCLUSION_TEXTURE;
        }
        if self.double_sided {
            flags |= CustomMaterialFlags::DOUBLE_SIDED;
        }
        if self.unlit {
            flags |= CustomMaterialFlags::UNLIT;
        }
        if self.fog_enabled {
            flags |= CustomMaterialFlags::FOG_ENABLED;
        }
        if self.depth_map.is_some() {
            flags |= CustomMaterialFlags::DEPTH_MAP;
        }
        #[cfg(feature = "pbr_transmission_textures")]
        {
            if self.specular_transmission_texture.is_some() {
                flags |= CustomMaterialFlags::SPECULAR_TRANSMISSION_TEXTURE;
            }
            if self.thickness_texture.is_some() {
                flags |= CustomMaterialFlags::THICKNESS_TEXTURE;
            }
            if self.diffuse_transmission_texture.is_some() {
                flags |= CustomMaterialFlags::DIFFUSE_TRANSMISSION_TEXTURE;
            }
        }
        ...
        #[cfg(feature = "pbr_anisotropy_texture")]
        {
            if self.anisotropy_texture.is_some() {
                flags |= CustomMaterialFlags::ANISOTROPY_TEXTURE;
            }
        }
        ...
        #[cfg(feature = "pbr_specular_textures")]
        {
            if self.specular_texture.is_some() {
                flags |= CustomMaterialFlags::SPECULAR_TEXTURE;
            }
            if self.specular_tint_texture.is_some() {
                flags |= CustomMaterialFlags::SPECULAR_TINT_TEXTURE;
            }
        }
        ...
        #[cfg(feature = "pbr_multi_layer_material_textures")]
        {
            if self.clearcoat_texture.is_some() {
                flags |= CustomMaterialFlags::CLEARCOAT_TEXTURE;
            }
            if self.clearcoat_roughness_texture.is_some() {
                flags |= CustomMaterialFlags::CLEARCOAT_ROUGHNESS_TEXTURE;
            }
            if self.clearcoat_normal_texture.is_some() {
                flags |= CustomMaterialFlags::CLEARCOAT_NORMAL_TEXTURE;
            }
        }
            ...
            if let Some(texture) = images.get(normal_map_id) {
                match texture.texture_format {
                    ...
                    | TextureFormat::EacRg11Unorm => {
                        flags |= CustomMaterialFlags::TWO_COMPONENT_NORMAL_MAP;
                    }
                    _ => {}
                }
            }
            if self.flip_normal_map_y {
                flags |= CustomMaterialFlags::FLIP_NORMAL_MAP_Y;
            }
        }
        // NOTE: 0.5 is from the glTF default - do we want this?
        let mut alpha_cutoff = 0.5;
        match self.alpha_mode {
            AlphaMode::Opaque => flags |= CustomMaterialFlags::ALPHA_MODE_OPAQUE,
            AlphaMode::Mask(c) => {
                alpha_cutoff = c;
                flags |= CustomMaterialFlags::ALPHA_MODE_MASK;
            }
            AlphaMode::Blend => flags |= CustomMaterialFlags::ALPHA_MODE_BLEND,
            AlphaMode::Premultiplied => flags |= CustomMaterialFlags::ALPHA_MODE_PREMULTIPLIED,
            AlphaMode::Add => flags |= CustomMaterialFlags::ALPHA_MODE_ADD,
            AlphaMode::Multiply => flags |= CustomMaterialFlags::ALPHA_MODE_MULTIPLY,
            AlphaMode::AlphaToCoverage => {
                flags |= CustomMaterialFlags::ALPHA_MODE_ALPHA_TO_COVERAGE;
            }
        };

        if self.attenuation_distance.is_finite() {
            flags |= CustomMaterialFlags::ATTENUATION_ENABLED;
        }
        ...
        CustomMaterialUniform {
            ...
            max_relief_mapping_search_steps: 1,
...
// bitflags! {
//     /// The pipeline key for `CustomMaterial`, packed into 64 bits.
...
impl Material for CustomMaterial {
    ...
    fn prepass_fragment_shader() -> ShaderRef {
        bevy::pbr::PBR_PREPASS_SHADER_HANDLE.into()
    }

    fn deferred_fragment_shader() -> ShaderRef {
        bevy::pbr::PBR_SHADER_HANDLE.into()
    }
