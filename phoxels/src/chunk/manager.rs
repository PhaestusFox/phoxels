...
pub struct ChunkGenerator {
    to_generate: IndexSet<ChunkId>,
    generating: HashMap<ChunkId, (Entity, Task<ChunkData>)>,
    old_generating: HashMap<ChunkId, (Entity, Task<ChunkData>)>,
}

impl ChunkGenerator {
    /// Adds a chunk to the queue to have its data generated.
    pub fn add_to_queue(&mut self, chunk_id: ChunkId) {
...
pub struct ChunkMesher {
    to_generate: IndexSet<ChunkId>,
    generating: HashMap<ChunkId, (Entity, Task<Mesh>)>,
    old_generating: HashMap<ChunkId, (Entity, Task<Mesh>)>,
}
...
impl ChunkManager {
    pub fn add_chunk(&mut self, chunk_id: ChunkId, entity: Entity) {
        self.loaded_chunks.insert(chunk_id, entity);
    }

    pub fn get_chunk(&self, chunk_id: &ChunkId) -> Option<Entity> {
        self.loaded_chunks.get(chunk_id).copied()
    }

    pub fn remove_chunk(&mut self, chunk_id: &ChunkId) -> Option<Entity> {
...
// );
pub struct PhoxelGenerator(Arc<dyn Fn(ChunkId) -> ChunkData + Send + Sync>);

impl PhoxelGenerator {
    pub fn new<F: Fn(ChunkId) -> ChunkData + Send + Sync + 'static>(f: F) -> Self {
        Self(Arc::new(f))
    }

    async fn generate(self, chunk_id: ChunkId) -> ChunkData {
        (self.0)(chunk_id)
    }
}
...
pub(super) fn start_generating_chunk_data(
    ...
    for _ in 0..generator.to_generate.len().min(can_generate) {
        ...
        let Some(entity) = chunk_manager.get_chunk(&chunk_id) else {
            #[cfg(feature = "log")]
            bevy::log::error!("ChunkManager does not have chunk: {:?}", chunk_id);
            continue;
        };
        let chunk_generator = if let Ok(generator) = chunk_specific_generators.get(entity) {
            generator
        ...
        generator.generating.insert(
            ...
            (
                entity,
                task_pool.spawn(chunk_generator.clone().generate(chunk_id)),
            ),
        );
    }
}
...
pub(super) fn start_generating_chunk_mesh(
    ...
    for _ in 0..generator.to_generate.len().min(can_generate) {
        ...
        generator.generating.insert(
            chunk_id,
            (entity, task_pool.spawn(chunk_data.clone().generate_mesh())),
        );
    }
}
...
pub(super) fn extract_finished_chunk_data(
    ...
    for (chunk_id, (entity, task)) in old_generating.drain() {
        if !task.is_finished() {
            generating.insert(chunk_id, (entity, task));
            continue;
        }
        #[cfg(feature = "log")]
        bevy::log::trace!("Extracting finished data for chunk: {:?}", chunk_id);
...
pub(super) fn extract_finished_chunk_mesh(
    ...
    for (chunk_id, (entity, task)) in old_generating.drain() {
        ...
        #[cfg(feature = "log")]
        bevy::log::trace!(
            "Chunk {:?} has finished meshing inserting mesh into {:?}",
            chunk_id,
            entity
        );
