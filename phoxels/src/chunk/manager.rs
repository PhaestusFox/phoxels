use std::{any::Any, fmt::Debug, panic, sync::Arc};
...
#[derive(Resource)]
pub struct PhoxelGenerator<T: PhoxelGeneratorData>(Arc<dyn Fn(T) -> ChunkData + Send + Sync>);

impl<T: PhoxelGeneratorData> Clone for PhoxelGenerator<T> {
    fn clone(&self) -> Self {
        Self(self.0.clone())
    }
}
...
impl<T: PhoxelGeneratorData> Component for PhoxelGenerator<T> {
...
impl<T: PhoxelGeneratorData> PhoxelGenerator<T> {
    pub fn new<F: Fn(T) -> ChunkData + Send + Sync + 'static>(f: F) -> Self {
        Self(Arc::new(f))
    }

    async fn generate(self, data: T) -> ChunkData {
        (self.0)(data)
    }
...
pub(super) fn start_generating_chunk_data<T: PhoxelGeneratorData>(
    default_generator: Option<Res<PhoxelGenerator<T>>>,
